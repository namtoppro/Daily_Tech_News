...
1. 배경, 원인, 결과, 의미를 중심으로 요약하세요. 전망 문장은 최소화하고, 가능하면 미래 예측보다 현재 확인된 사실과 해석에 집중하세요.
2. 미래 표현이 꼭 필요할 때도 `시사한다`, `보여준다`, `해석된다`, `가능성이 있다` 정도로 제한하고, `~할 것이다`, `~이끌 것이다`, `~도래할 것이다`, `핵심으로 자리 잡을 전망이다` 같은 강한 단정은 사용하지 마세요.
3. 계층 구조화:
   - 🚨 HEADLINE (1~2개): 가장 상세하게 작성. 한국 독자 관점에서 파급력, 클릭성, 산업 영향도를 우선 반영하세요.
   - 🔥 MAJOR NEWS (4~6개): 핵심 논조 유지
   - 📄 BRIEF: 간결 요약
4. 각 기사 하단에 [Source: 언론사명] 표기
5. 템플릿 문장 반복을 줄이고 기사별 어조를 약간씩 다르게 하세요.
6. 카드 길이 제한:
   - HEADLINE summary: 220~280자 수준
   - MAJOR summary: 120~170자 수준
   - BRIEF summary: 70~110자 수준
7. 과장광고, 과도한 확신, 선정적 표현 금지
8. 브리핑은 기사처럼 보여야 하며, 과하게 홍보성/예언형으로 쓰지 마세요.
...
- 그 아래 섹션 제목은 `일간 기술 브리핑`, `오늘의 핵심 이슈`처럼 오늘 기준 표현만 사용
