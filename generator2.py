...
1. 각 카드 내용은 **확인된 사실 + 짧은 의미 해석** 구조로 작성하세요. 사실과 해석이 섞이더라도 문맥상 구분이 보이게 쓰세요.
2. 요약은 배경, 원인, 결과, 의미를 중심으로 작성하고, 전망 문장은 최소화하세요. 가능하면 미래 예측보다 현재 확인된 사실과 해석에 집중하세요.
3. 미래 표현이 꼭 필요할 때도 `시사한다`, `보여준다`, `해석된다`, `가능성이 있다` 정도로 제한하고, `~할 것이다`, `~이끌 것이다`, `~도래할 것이다`, `핵심으로 자리 잡을 전망이다` 같은 강한 단정은 사용하지 마세요.
4. 계층 구조화:
   - 🚨 HEADLINE (1~2개): 가장 상세하게 작성. 한국 독자 관점에서 파급력, 클릭성, 산업 영향도를 우선 반영하세요.
   - 🔥 MAJOR NEWS (4~6개): 핵심 논조 유지
   - 📄 BRIEF: 간결 요약
5. 각 기사 하단에 [Source: 언론사명] 표기
6. 템플릿 문장 반복을 줄이고 기사별 어조를 약간씩 다르게 하세요.
7. 카드 길이 제한:
   - HEADLINE summary: 220~280자 수준
   - MAJOR summary: 120~170자 수준
   - BRIEF summary: 70~110자 수준
8. 과장광고, 과도한 확신, 선정적 표현 금지
9. 브리핑은 기사처럼 보여야 하며, 과하게 홍보성/예언형으로 쓰지 마세요.
...
- 각 핵심 이슈는 반드시 아래 3개 구역으로 나눠라:
  ...
  3. **AI 인사이트** → 여기에는 왜 중요한지, 어떤 흐름인지, 누가 영향을 받는지 해석한다.
- `발표 및 업데이트 내용`에는 해석이나 의견을 넣지 마라.
- `AI 인사이트`에는 기사 원문에 없는 추가 맥락 해석은 가능하지만, 사실처럼 단정하지 말고 시사점/가능성 수준으로 제한하라.
- 수치/버전/날짜 명시
