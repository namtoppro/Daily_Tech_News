...
10. 카드 길이 제한은 권장치일 뿐, 핵심 사실 보존보다 우선하지 마세요.
   - HEADLINE summary: 220~280자 권장
   - MAJOR summary: 120~170자 권장
   - BRIEF summary: 70~110자 권장
11. 길이를 줄이더라도 아래는 가능한 한 유지하세요:
   - 누가 발표/행동했는지
   - 무엇이 바뀌었는지
   - 핵심 수치/날짜/규모
   - 왜 중요한지
12. `[인사이트]`를 넣기 위해 본문 사실 요약을 과도하게 줄이지 마세요. `[인사이트]`를 제거해도 기사 핵심이 남아 있어야 합니다.
13. 과장광고, 과도한 확신, 선정적 표현 금지
14. 브리핑은 기사처럼 보여야 하며, 과하게 홍보성/예언형으로 쓰지 마세요.
...
- 각 핵심 이슈는 반드시 아래 3개 구역으로 나눠라:
  1. **문제 인식**
  2. **발표 및 업데이트 내용** → 여기에는 기사에 나온 사실, 수치, 날짜, 발언만 쓴다. 길이를 줄이더라도 누가/무엇을/수치/왜 중요한지가 빠지면 안 된다.
