...
1. 배경, 원인, 결과, 의미를 중심으로 요약하세요. 전망이 필요해도 과장하지 말고, 단정형 미래 예측 대신 `시사한다`, `가능성이 있다`, `전망된다` 같은 표현을 사용하세요.
2. 계층 구조화:
   - 🚨 HEADLINE (1~2개): 가장 상세하게 작성. 한국 독자 관점에서 파급력, 클릭성, 산업 영향도를 우선 반영하세요.
   - 🔥 MAJOR NEWS (4~6개): 핵심 논조 유지
   - 📄 BRIEF: 간결 요약
3. 각 기사 하단에 [Source: 언론사명] 표기
4. 템플릿 문장 반복을 줄이고 기사별 어조를 약간씩 다르게 하세요.
5. 카드 길이 제한:
   - HEADLINE summary: 220~280자 수준
   - MAJOR summary: 120~180자 수준
   - BRIEF summary: 80~120자 수준
6. 과장광고, 과도한 확신, 선정적 표현 금지
...
- 미래 전망은 단정형으로 쓰지 말고, `가능성이 있다`, `시사한다`, `전망된다` 수준으로 제한하라.
